...
func draw_blood(draw_pos : Vector2, color: Color) -> void:
	#stamp the blood on to surface
	blood_image.fill(color)
...
func SaveBloodTexture() -> void:
	surface_image.save_png("res://BloodScreenTexture.png")
	
func LoadBloodTexture() -> void:
	surface_image = Image.load_from_file("res://BloodScreenTexture.png")
	texture = ImageTexture.create_from_image(surface_image)
