...
func upgrade_building(building: Building) -> void:
	...
	for upgrade: BuildingUpgrade in upgrades:
		if upgrade.upgrade_name == "Speed":
			...
			trap_speed = base_trap_speed * 1.5**(upgrade.upgrade_level-1)
			animation_player.speed_scale = trap_speed / base_trap_speed
		elif upgrade.upgrade_name == "Cooldown":
			# 50% decrease each level
			# formula: cd - cd(1 - (1/2)^(x-1)) 
			#attack_cooldown = base_cooldown - base_cooldown *(1 - 0.5**(upgrade.upgrade_level-1))
			attack_cooldown = base_cooldown * 0.5**(upgrade.upgrade_level-1)
